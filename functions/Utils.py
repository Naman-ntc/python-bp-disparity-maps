...
def ReturnImages(imageNum):
	imgLeft = cv2.imread('inputs/left' + str(imageNum) + ('.bmp' if imageNum ==3 else '.png'))
	imgLeft = cv2.cvtColor(imgLeft, cv2.COLOR_BGR2GRAY)
	imgRight = cv2.imread('inputs/right' + str(imageNum) + ('.bmp' if imageNum ==3 else '.png'))
...
def FillFacLists(HorFactorNodes, VerFactorNodes, AllVariableNodes, M, N, pairwiseCostMatrix, opts):
	for i in range(M-1):
		...
		for j in range(N-1):
			...
			temp1.append(PairWiseFactorNode(opts.maxdisp, opts.smoothening, pairwiseCostMatrix))
			AllVariableNodes[i][j].pushfac([temp1[-1],0])
			AllVariableNodes[i][j+1].pushfac([temp1[-1],1])
			
			temp2.append(PairWiseFactorNode(opts.maxdisp, opts.smoothening, pairwiseCostMatrix))
